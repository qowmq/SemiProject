   ...
   protected void doGet(HttpServletRequest request, HttpServletResponse response)
      ...
      if (cmd.contentEquals("/confirm.file")) {
         ...
         try {
            ...
            List<File_ListDTO> allFileList = new ArrayList<>();
            allFileList = FileListDAO.getInstance().selectAll(recordNum);

            System.out.println("그사람 파일 전부 가져와!!! ::: " + allFileList.size());
            int result = 0;

            if (allFileList.size() != 0) {
               for (File_ListDTO item : allFileList) {
                  if (item.getUpload_date().contentEquals(date)) {
                     int seq = FileListDAO.getInstance().getSeq();
                     System.out.println(seq);
                     result = FileListDAO.getInstance().update(seq, dto);

                     System.out.println("업데이트로 해쪄요! ㅎㅎㅎ : "+result);
                  } else {
