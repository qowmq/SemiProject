...
public class Member extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (realPath.contentEquals("/loginPage.mem")) {
		...
		} else if (realPath.contentEquals("/login.mem")) {
			try {
				...
				if (loginResult) {
					...
					request.getRequestDispatcher("user/login/logincheck.jsp").forward(request, response);
				} else {
					// 일반 사용자 체크용
			...
			try {
				...
				dao.insert(new MemberDTO(0, id, pw, name, phone, email, 0, null, 0));
...
//					char cValue3 = (char)((dValue * 26) + 97); // 소문자
			...
			try {
				...
				int start = cpage * adminboardCongiuration.Configuration.recordCountPerPage
						- (adminboardCongiuration.Configuration.recordCountPerPage - 1);
